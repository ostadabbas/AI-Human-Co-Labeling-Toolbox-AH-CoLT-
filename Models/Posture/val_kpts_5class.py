...
import argparse
import numpy as np
from load_custom_dataset import load_kpts_5class
from load_custom_dataset import load_kpts_5class_gt
from models import *
...
def plotCM(array):
    df_cm = pd.DataFrame(array, ["Supine", "Prone", "Sitting", "Standing", "All Fours"], ["Supine", "Prone", "Sitting", "Standing", "All Fours"])
...
def test(epoch, dir_folder):
    ...
    with torch.no_grad():
        ...
        print('/n')
        print('allfours: ', right_allfours/num_allfours, num_allfours)
    ...
    print(new_tar)
    cm = confusion_matrix(new_tar, new_pred, [0, 1, 2, 3, 4])
    print(cm)
    plotCM(cm)
    ...
    np.save(os.path.join(dir_folder, 'score.npy'), score_list)
...
if __name__ == "__main__":
   ...
   #                     default='/home/faye/Documents/InfantProject/data/MIMM_2D/annotations/mimm/person_keypoints_validate_infant.json',
    ...
    testset = load_kpts_5class(args.test_pred, args.test_anno, transforms = None)
    ...
    checkpoint = torch.load('/home/faye/Documents/FiDIP_Posture/Classifier/train_syrip_kpts_output_202207310722/kpts_ckpt.pth') # previous best 4 posture classes model
    #checkpoint = torch.load('/home/faye/Documents/FiDIP_Posture/Classifier/train_syrip_withoutTrans_kpts_output_202207310733/kpts_ckpt.pth') # previous best 4 posture classes model
